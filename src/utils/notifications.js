...
/**
 * Muestra una notificación toast centrada en pantalla.
 * @param {string} message - Mensaje a mostrar.
 * @param {"success"|"error"|"info"} [type="success"] - Tipo de notificación.
 * @param {number} [duration=3000] - Duración en milisegundos antes de auto-cerrar.
 */
export const showNotification = (message, type = "success", duration = 3000) => {
  ...
  const styles = {
    success: {
      bg: "bg-green-600",
      icon: '<svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>',
    },
    error: {
      bg: "bg-red-600",
      icon: '<svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>',
    },
    info: {
      bg: "bg-blue-600",
...
/**
 * Crea un overlay modal con backdrop blur.
 * @returns {HTMLDivElement}
 */
const createModalOverlay = () => {
  const overlay = document.createElement("div");
  overlay.className =
    "fixed inset-0 z-[3000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in";
  return overlay;
};

/**
 * Objeto con métodos para mostrar diálogos modales de confirmación y prompt.
 */
export const dialog = {
  /**
   ...
   * @param {string} title - Título del diálogo.
   * @param {string} message - Mensaje/instrucción del diálogo.
   * @param {string} [defaultValue=""] - Valor por defecto del input.
   * @returns {Promise<string|null>} Texto ingresado o null si se canceló.
   */
...
/**
 * Objeto para mostrar y ocultar un preloader global de pantalla completa.
 */
export const preloader = {
  /**
   * Muestra el preloader global con un mensaje personalizado.
  ...
  show: (message = "Cargando...") => {
    ...
    overlay.className =
      "fixed inset-0 z-[4000] bg-black flex flex-col items-center justify-center animate-fade-in backdrop-blur-sm";
    overlay.innerHTML = `
  ...
  /**
   * Oculta el preloader global con animación de fade-out.
   */
  hide: () => {
...
/**
 * Atajos rápidos para mostrar notificaciones por tipo.
 */
export const toast = {
  /**
