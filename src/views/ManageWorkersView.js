...
export class ManageWorkersView {
  ...
  render(workers, acciones, companies = []) {
    ...
    const template = html`
            ...
            <section class="space-y-12 scroll-mt-24" id="worker-form-section">
              ...
              <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                  ...
                  <div class="flex items-center justify-between px-2">
                    <div class="flex items-center gap-4">
                      ...
                      <span class="text-[10px] font-black uppercase tracking-[0.3em] text-stone-900"
                        >Personal Registrado (${this.allWorkers.length})</span
                      >
                    </div>
                ...
                <div class="lg:col-span-5 order-1 lg:order-2">
                  <div
                    class="bg-white p-8 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] border border-stone-100 shadow-xl lg:sticky lg:top-10 overflow-hidden"
                    id="worker-editor-container"
                  >
                    <div class="relative z-10">
                      ...
                      >
                        ...
                        <div class="space-y-6">
                          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                id="worker-name"
                                placeholder="Ej: Juan"
                                class="w-full bg-stone-50 border-b-2 border-stone-100 py-3 text-stone-900 text-base focus:border-primary transition-all outline-none"
                                required
                              />
                            </div>
                            <div>
                          ...
                          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <select
                                id="worker-company"
                                class="w-full bg-stone-50 border-b-2 border-stone-100 py-3 text-stone-900 text-sm focus:border-primary outline-none transition-all appearance-none"
                                required
                              >
                          ...
                          <label
                            class="flex items-center gap-4 cursor-pointer group p-4 bg-stone-50 rounded-2xl border border-stone-100 hover:border-primary/30 transition-all active:scale-[0.98]"
                          >
                            <input
                              type="checkbox"
                              id="worker-is-field-manager"
                              class="w-5 h-5 rounded-lg border-stone-200 bg-white text-primary focus:ring-0 focus:ring-offset-0"
                            />
                            <div class="flex flex-col">
                              <span
                                class="text-[11px] uppercase tracking-widest text-stone-600 group-hover:text-primary transition-colors font-black"
                                >Encargado de Campo</span
                              >
                              <span class="text-[8px] text-stone-400 italic"
                                >Habilita raciones grupales</span
                              >
                            </div>
                          </label>
                          ...
                          <!-- CONTROL BIOMÉTRICO -->
                          <div
                            class="p-6 bg-stone-50 rounded-[2rem] border border-stone-100 space-y-6 text-center shadow-inner"
                          >
                            <p
                              class="text-[10px] font-black uppercase text-primary tracking-[0.3em]"
                            >
                              Control Biométrico
                            </p>
                            ...
                            <div class="flex justify-center gap-3">
                              ${[1, 2, 3].map(
                                (i) =>
                                  html`<div
                                    id="step-dot-${i}"
                                    class="h-6 w-6 rounded-full border-2 border-stone-200 bg-white transition-all flex items-center justify-center text-[10px] font-black text-stone-300"
                                  >
                                    ${i}
                                  </div>`
                              )}
                            ...
                            <div
                              class="flex flex-col items-center gap-3 py-4 px-6 rounded-2xl bg-white border border-stone-100"
                            >
                              <div class="flex items-center gap-3">
                                <div
                                  id="status-dot"
                                  class="h-2.5 w-2.5 rounded-full bg-red-400 shadow-[0_0_8px_rgba(248,113,113,0.3)]"
                                ></div>
                                <span
                                  id="status-text"
                                  class="text-[11px] font-black text-stone-400 uppercase tracking-widest"
                                  >Esperando inicio</span
                                >
                              </div>
                              <p
                                id="status-instruction"
                                class="text-[9px] font-black text-primary hidden uppercase animate-pulse"
                              >
                                Coloque su dedo...
                              </p>
                            </div>
                            ...
                            <button
                              type="button"
                              id="capture-fingerprint-btn"
                              @click=${() => this._handleCapture()}
                              class="w-full bg-stone-900 text-white py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-primary transition-all active:scale-95 shadow-lg"
                            >
                              <svg
                                class="h-5 w-5"
                                viewBox="0 0 24 24"
                        ...
                        <div class="flex flex-col gap-4 pt-6 border-t border-stone-50">
                          ...
                          <button
                            type="button"
                            id="cancel-worker-edit"
                            @click=${() => this.resetForm()}
                            class="hidden w-full text-stone-400 py-3 text-xs font-bold uppercase tracking-widest hover:text-stone-900 transition-all italic text-center"
                          >
                            Descartar Cambios
                          </button>
                        </div>
      ...
      <!-- MODAL DE DETALLES -->
      <div
        id="worker-detail-modal"
        class="hidden fixed inset-0 z-[3000] flex items-center justify-center p-4"
      >
        <div
        ...
        ></div>
        <div
          class="relative bg-white w-full max-w-2xl max-h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-scale-in"
        >
          <div
            class="p-8 sm:p-10 border-b border-stone-50 flex justify-between items-center shrink-0"
          >
            <div>
              <h2 class="text-xl sm:text-2xl font-display italic text-stone-950">
                Ficha del Personal
              </h2>
              <p class="text-[9px] text-stone-400 uppercase tracking-widest mt-1">
                Historial y reportes individuales
              </p>
            </div>
            <button
              @click=${() => this.toggleModal(false)}
              class="p-3 rounded-full hover:bg-stone-50 transition-colors"
            >
  ...
  _getFilteredWorkers() {
    const q = String(this.filters.query || "")
      .toLowerCase()
      .trim();
    ...
    return this.allWorkers.filter((w) => {
      ...
      const matchesQuery =
        !q || workerDni.includes(q) || workerName.includes(q) || workerLastName.includes(q);
  ...
  _renderWorkerRow(w) {
    ...
    const nombreCompleto =
      `${apellidosStr}${apellidosStr && nombreStr ? ", " : ""}${nombreStr}` || "Sin Nombre";
    ...
    return html`
      <div
        class="group relative overflow-hidden rounded-[2.5rem] border-2 border-stone-50 bg-white p-6 sm:p-8 shadow-sm hover:shadow-2xl transition-all duration-700 flex flex-col md:flex-row items-center gap-6 cursor-pointer"
        @click=${() => this.acciones.onViewDetails(w.id)}
      >
        <div
          class="h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-stone-950 flex items-center justify-center text-white font-display italic text-2xl shadow-xl shrink-0 group-hover:bg-primary transition-all duration-500"
        >
          ${inicial}
        </div>
        ...
        <div class="flex-1 min-w-0 text-center md:text-left">
          <div class="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
            <h4
              class="text-base sm:text-lg font-sans font-bold text-stone-900 uppercase tracking-tight truncate"
            >
              ${nombreCompleto}
            </h4>
            ${w.esEncargadoCampo
              ? html`<span
                  class="text-[7px] font-black text-amber-600 uppercase bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100 tracking-tighter"
                  >Encargado de Campo</span
                >`
              : ""}
          </div>
          <div class="flex flex-wrap justify-center md:justify-start items-center gap-x-4 gap-y-1">
            <span class="text-[9px] font-black uppercase text-primary tracking-widest"
              >${empresaStr}</span
            >
        ...
        <div
          class="shrink-0 flex items-center gap-2 bg-stone-50 px-4 py-2 rounded-full border border-stone-100"
        >
          <div
            class="h-1.5 w-1.5 rounded-full ${huellaCount >= 3
        ...
        <div class="flex gap-2 shrink-0 relative z-20">
          <button
            @click=${(e) => {
              e.stopPropagation();
              this.acciones.onEdit(w.id);
            }}
            class="p-3.5 rounded-2xl bg-stone-50 text-stone-500 hover:bg-stone-950 hover:text-white transition-all active:scale-90 border border-stone-100"
          >
            <svg
              class="h-5 w-5"
              fill="none"
          ...
          <button
            @click=${(e) => {
              e.stopPropagation();
              this._handleDelete(w.id);
            }}
            class="p-3.5 rounded-2xl bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all active:scale-90 border border-red-100"
          >
            <svg
              class="h-5 w-5"
              fill="none"
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const result = await onCapture((step) => {
          if (step === "captured") {
            ...
            dot.className =
              "h-3 w-3 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]";
          }
        });
  ...
  updateStepDots(count) {
    for (let i = 1; i <= 3; i++) {
      ...
      dot.className =
        i <= count
          ? "h-6 w-6 rounded-full border-2 border-emerald-500 bg-emerald-500 text-white flex items-center justify-center text-[10px] font-black shadow-lg"
          : "h-6 w-6 rounded-full border-2 border-stone-200 bg-white text-stone-300 flex items-center justify-center text-[10px] font-black";
    }
  }
  ...
  updateFingerprintStatus(hasHuellas) {
    ...
    if (!dot || !text) return;
    if (hasHuellas) {
      dot.className = "h-3 w-3 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]";
      text.textContent = "Huellas Listas (3/3)";
    } else {
      dot.className = "h-3 w-3 rounded-full bg-red-400";
      text.textContent = "Sin huellas";
    }
  }
  ...
  showWorkerDetails(worker, attendance, reportActions) {
    ...
    const modalTemplate = html`
      <div class="space-y-10">
        <div
          class="flex flex-col sm:flex-row items-center gap-6 p-6 bg-stone-50 rounded-[2.5rem] border border-stone-100 text-center sm:text-left"
        >
          <div
            class="h-20 w-20 rounded-full bg-stone-950 flex items-center justify-center text-white text-3xl font-display italic shadow-xl"
          >
            ${worker.nombre.charAt(0)}
          </div>
          <div class="flex-1 min-w-0">
            <h3 class="text-xl font-sans font-bold text-stone-900 uppercase truncate">
            ...
            <div class="flex flex-wrap justify-center sm:justify-start gap-3 items-center mt-2">
              <span class="text-[10px] font-black text-primary uppercase tracking-widest"
                >${worker.empresa || "Particular"}</span
              >
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ${[
            ...
            { l: "Des.", v: stats.desayunos, c: "bg-white text-stone-900" },
            { l: "Alm.", v: stats.almuerzos, c: "bg-white text-stone-900" },
            { l: "Cena", v: stats.cenas, c: "bg-white text-stone-900" },
          ].map(
            (s) =>
              html`<div class="${s.c} p-5 rounded-3xl text-center shadow-sm border border-stone-50">
                <span class="text-[8px] font-black uppercase tracking-widest opacity-60 mb-1 block"
                  >${s.l}</span
                ><span class="text-2xl font-black tracking-tighter">${s.v}</span>
              </div>`
          )}
        </div>
        <div
          class="p-6 sm:p-8 bg-white border border-stone-100 rounded-[2.5rem] shadow-sm space-y-6"
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="text-[9px] font-black text-stone-400 uppercase block mb-2">Inicio</label
              ><input
                type="date"
                id="modal-start"
                class="w-full bg-stone-50 border-stone-100 rounded-xl py-2 px-4 text-xs font-bold"
              />
            </div>
            <div>
              <label class="text-[9px] font-black text-stone-400 uppercase block mb-2">Fin</label
              ><input
                type="date"
                id="modal-end"
                class="w-full bg-stone-50 border-stone-100 rounded-xl py-2 px-4 text-xs font-bold"
          ...
          <div class="flex gap-3">
            <button
              @click=${() => this._handleModalDownload(worker, attendance, onDownloadPdf, "pdf")}
              class="flex-1 bg-stone-950 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl"
            >
              PDF
            </button>
            <button
              @click=${() =>
                this._handleModalDownload(worker, attendance, onDownloadExcel, "excel")}
              class="flex-1 bg-stone-100 text-stone-600 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-stone-200"
            >
              EXCEL
            </button>
          </div>
        </div>
        <div class="overflow-hidden rounded-3xl border border-stone-100 bg-white">
          <table class="w-full text-left min-w-[350px]">
            <thead class="bg-stone-50 text-[9px] font-black uppercase text-stone-400">
              <tr class="border-b border-stone-100">
                <th class="px-6 py-4">Fecha</th>
                <th class="px-6 py-4 text-right">Servicio</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-stone-50">
              ${attendance.slice(0, 10).map(
                (reg) =>
                  html`<tr class="text-[10px] font-bold text-stone-600">
                    <td class="px-6 py-4">${reg.fecha}</td>
                    <td class="px-6 py-4 text-right">
                      <span
                        class="px-2 py-0.5 rounded-md bg-stone-50 text-[8px] font-black uppercase"
                        >${reg.tipo}</span
                      >
                    </td>
                  </tr>`
  ...
  _handleModalDownload(worker, attendance, callback, format) {
    const s = document.getElementById("modal-start").value,
      e = document.getElementById("modal-end").value;
    let f = [...attendance];
    if (s) f = f.filter((a) => a.fecha >= s);
    if (e) f = f.filter((a) => a.fecha <= e);
