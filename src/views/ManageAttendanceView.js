...
export class ManageAttendanceView {
        ...
        <div class="max-w-[1400px] mx-auto px-4 py-8 sm:py-16">
          ...
          <header class="flex flex-col sm:flex-row sm:items-end justify-between gap-8 mb-16 sm:mb-20 pb-8 border-b border-stone-200">
            <div class="space-y-4 sm:space-y-6">
                <button @click=${acciones.onBack} class="flex items-center gap-3 text-primary hover:text-stone-900 font-bold text-[9px] uppercase tracking-[0.5em] transition-all group">
                ...
                <div class="flex flex-col gap-2">
                    <span class="text-[8px] uppercase tracking-[0.4em] text-stone-400 font-bold">Control de Alimentación</span>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-8 lg:gap-12 items-start">
                    ...
                    <div class="space-y-8 bg-white p-6 sm:p-8 rounded-[2.5rem] border border-stone-100 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <input type="date" id="filter-date" .value=${this.filters.date} 
                                ...
                                <select id="filter-company-list" 
                                    ...
                                    ${this.companies.map(c => html`
                                        <option value="${c.nombre}" ?selected=${this.filters.company === c.nombre}>${c.nombre}</option>
                                    `)}
                                </select>
                            </div>
                        </div>
                ...
                <div class="xl:col-span-4 order-1 xl:order-2 space-y-8 lg:sticky lg:top-10">
                            ...
                            <div class="space-y-5">
                                <div>
                                    ...
                                    <select id="attendance-worker-dni" @change=${(e) => this._handleWorkerChange(e)} class="w-full bg-stone-50 border-2 border-stone-100 rounded-2xl py-4 px-5 text-stone-900 text-sm focus:border-primary outline-none transition-all appearance-none cursor-pointer" required>
                                        ...
                                        ${this.allWorkers.sort((a,b) => a.apellidos.localeCompare(b.apellidos)).map(w => html`
                                            <option value="${w.dni}">${w.apellidos}, ${w.nombre} (${w.dni})</option>
                                        `)}
                                    </select>
                                </div>

                                <div id="local-consumption-toggle" class="hidden bg-stone-50/50 p-4 rounded-2xl border border-stone-100 flex items-center justify-between">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-black uppercase text-stone-700">Consumo Local</span>
                                        <span class="text-[8px] text-stone-400 uppercase tracking-tighter italic">¿Comerá aquí?</span>
                                    </div>
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="attendance-also-eats" checked @change=${() => this._updateCampoUI()} class="sr-only peer">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="text-[10px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-2 block">Fecha</label>
                                        <input type="date" id="attendance-date" .value=${getLocalDateString()} class="w-full bg-stone-50 border-b-2 border-stone-100 py-3 text-stone-900 text-sm focus:border-primary outline-none transition-all" required />
                                    </div>
                                    <div>
                                        <label class="text-[10px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-2 block">Servicio</label>
                                        <select id="attendance-type" class="w-full bg-stone-50 border-b-2 border-stone-100 py-3 text-stone-900 text-sm focus:border-primary outline-none transition-all appearance-none disabled:opacity-30 disabled:grayscale">
                                ...
                                <div id="field-manager-options" class="hidden p-6 bg-amber-50/40 rounded-3xl border border-amber-100 space-y-4 text-center">
                                    <label class="text-[9px] uppercase tracking-[0.3em] text-amber-700 font-black mb-1 block">Raciones a Campo</label>
                    ...
                    <div class="bg-white p-8 sm:p-10 rounded-[2.5rem] border border-stone-100 shadow-xl space-y-8">
                        ...
                        <div class="space-y-6">
                            <div>
                                <label class="text-[10px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-3 block">Empresa</label>
                                <select id="report-company" class="w-full bg-stone-50 border-b-2 border-stone-100 py-3 text-sm font-bold text-stone-900 outline-none">
                                    <option value="">Consolidado General</option>
                                    ${this.companies.map(c => html`<option value="${c.nombre}">${c.nombre}</option>`)}
                                </select>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div><label class="text-[10px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-2 block">Desde</label><input type="date" id="report-start-date" class="w-full bg-stone-50 border-b-2 border-stone-100 py-2 text-xs font-bold outline-none" /></div>
                                <div><label class="text-[10px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-2 block">Hasta</label><input type="date" id="report-end-date" class="w-full bg-stone-50 border-b-2 border-stone-100 py-2 text-xs font-bold outline-none" /></div>
                            </div>
                            <div class="p-6 bg-stone-50 rounded-3xl border border-stone-100 space-y-4">
                                <span class="text-[8px] font-black uppercase text-stone-300 tracking-[0.3em] block text-center">Precios Liquidación (S/)</span>
                                <div class="grid grid-cols-3 gap-3">
                                    <div class="text-center bg-white p-2 rounded-xl border border-stone-100"><span class="text-[7px] font-bold text-stone-400 uppercase block mb-1">Des.</span><input type="number" id="price-d" value="10.00" step="0.10" class="w-full bg-transparent py-1 text-xs text-center font-black text-primary outline-none" /></div>
                                    <div class="text-center bg-white p-2 rounded-xl border border-stone-100"><span class="text-[7px] font-bold text-stone-400 uppercase block mb-1">Alm.</span><input type="number" id="price-a" value="12.00" step="0.10" class="w-full bg-transparent py-1 text-xs text-center font-black text-primary outline-none" /></div>
                                    <div class="text-center bg-white p-2 rounded-xl border border-stone-100"><span class="text-[7px] font-bold text-stone-400 uppercase block mb-1">Cena</span><input type="number" id="price-c" value="10.00" step="0.10" class="w-full bg-transparent py-1 text-xs text-center font-black text-primary outline-none" /></div>
                                </div>
                            </div>
                            <div class="flex flex-col gap-4 pt-4 border-t border-stone-50">
                                <button @click=${() => this._handleExport('pdf')} class="w-full bg-stone-950 text-white py-6 rounded-3xl text-xs font-black uppercase tracking-[0.4em] shadow-xl hover:bg-primary transition-all">Exportar PDF</button>
                                <button @click=${() => this._handleExport('excel')} class="w-full bg-stone-50 text-emerald-700 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-emerald-100 transition-all border border-emerald-100">Exportar Excel</button>
                            </div>
                        </div>
                    </div>
                </div>
  ...
  _renderAttendanceTable(attendances) {
    if (!attendances || attendances.length === 0) {
        return html`<div class="py-24 text-center border-2 border-dashed border-stone-100 rounded-[3rem] text-stone-300 uppercase tracking-[0.4em] text-[10px] font-bold italic bg-white">Sin registros</div>`;
    }
    return html`<div class="space-y-4">${attendances.map(a => this._renderAttendanceRow(a))}</div>`;
  }
  ...
  _renderAttendanceRow(a) {
    ...
    return html`
        <div class="group relative rounded-[2.5rem] border-2 border-stone-100 bg-white p-6 shadow-sm hover:shadow-2xl transition-all duration-700 flex flex-col xl:flex-row items-center gap-8">
            <div class="flex-1 min-w-0 w-full">
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    <div class="h-3 w-3 rounded-full ${a.tipo === 'Almuerzo' ? 'bg-primary' : a.tipo === 'Desayuno' ? 'bg-amber-500' : 'bg-indigo-600'}"></div>
                    <h4 class="text-base sm:text-lg font-black text-stone-900 uppercase tracking-tight truncate">${a.nombreCompleto}</h4>
            ...
            <div class="flex flex-col gap-2 min-w-[200px] w-full xl:w-auto bg-stone-50/50 p-4 rounded-[1.5rem] border border-stone-100">
                <div class="flex items-center gap-2 mb-1">
                    <span class="px-2 py-1 rounded-md ${statusColor} text-[7px] font-black uppercase border tracking-widest">${statusLabel}</span>
                ...
                <div class="text-[10px] font-bold text-stone-700">
                    ...
                    <span class="text-stone-400 font-mono font-normal ml-1">${formatTimestamp(a.updatedAt || a.createdAt || a.timestamp)}</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-6 w-full xl:w-auto justify-between border-t xl:border-t-0 xl:border-l border-stone-100 pt-6 xl:pt-0 xl:pl-8">
                <div class="flex flex-col items-end gap-2">
                    ${hasLocal ? html`<span class="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[9px] font-black uppercase tracking-tighter border border-emerald-100">Local</span>` : ''}
                    ${hasField ? html`<span class="px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-[9px] font-black uppercase tracking-tighter border border-amber-100">Campo: ${a.cantidadCampo}</span>` : ''}
                </div>
                <div class="flex gap-2">
                    <button @click=${() => this.acciones.onEdit(a.id)} class="p-4 rounded-[1.25rem] bg-stone-50 text-stone-400 hover:bg-stone-950 hover:text-white transition-all border border-stone-100 shadow-sm"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                    <button @click=${() => this._handleDelete(a.id)} class="p-4 rounded-[1.25rem] bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all border border-red-100 shadow-sm"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                </div>
            </div>
        </div>
    `;
  ...
  _handleWorkerChange(e) { this._updateCampoUI(); }
...
_handleFormSubmit(e) {
  ...
  async _handleExport(format) {
    ...
    const prices = { d: parseFloat(document.getElementById("price-d").value) || 10, a: parseFloat(document.getElementById("price-a").value) || 12, c: parseFloat(document.getElementById("price-c").value) || 10 };
  ...
  _resetForm() {
    ...
    document.getElementById("local-consumption-toggle").classList.add("hidden");
    const serviceSelect = document.getElementById("attendance-type");
    if (serviceSelect) { serviceSelect.disabled = false; serviceSelect.classList.remove("opacity-30", "grayscale"); }
  }

  async _handleDelete(id) { if (await dialog.confirm("Eliminar", "¿Está seguro?")) this.acciones.onDelete(id); }
