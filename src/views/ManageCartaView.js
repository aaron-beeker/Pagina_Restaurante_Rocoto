...
import {
  CATEGORIAS_SOLO_MENU_DIARIO,
  esProductoSoloMenuDiario,
} from "../constants/menuCategories.js";
...
export class ManageCartaView {
          ...
          <div class="space-y-24 sm:space-y-32">
            ...
            <section class="space-y-10 sm:space-y-12 scroll-mt-24" id="form-editor-section">
              ...
              <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                ...
                <div class="lg:col-span-7 order-2 lg:order-1 flex flex-col gap-6">
                  ...
                  <div
                    id="table-container"
                    class="lg:max-h-[1100px] lg:overflow-y-auto lg:pr-4 custom-scrollbar"
                  >
                ...
                <div class="lg:col-span-5 order-1 lg:order-2">
                  <div
                    class="bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 border border-stone-100 shadow-xl lg:sticky lg:top-8"
                  >
                    <div class="mb-5 sm:mb-6 border-b border-stone-50 pb-5">
                      <h4 class="text-stone-900 font-display italic text-base sm:text-lg">
                        Añadir o Editar Producto
                      </h4>
                      <p
                        class="text-[7px] sm:text-[8px] text-stone-400 uppercase tracking-widest mt-0.5"
                      >
            ...
            <section class="space-y-10">
              ...
              <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                <!-- Listado Izquierda -->
                <div
                  class="lg:col-span-7 lg:max-h-[450px] lg:overflow-y-auto lg:pr-4 custom-scrollbar"
                >
                  ${this._renderCategoriesGrid(categorias, acciones)}
                </div>
                <!-- Formulario Derecha -->
                <div class="lg:col-span-5">${this._renderCategoryForm(acciones)}</div>
              </div>
            </section>
          </div>
        </div>
    ...
    return html`
      <div class="space-y-2 sm:space-y-3">
        ${categorias.map(
          (cat, index) => html`
            <div
              class="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-3xl bg-white border-2 border-stone-50 shadow-sm group hover:border-primary/20 hover:shadow-xl transition-all duration-700 ${cat.activo ===
              false
                ? "bg-stone-50/50 grayscale-[0.5]"
                : ""}"
            >
              <!-- Reordenar y Acciones -->
              <div
                class="flex w-full sm:w-auto items-center justify-between sm:flex-col sm:gap-1.5 shrink-0 bg-stone-50 sm:bg-stone-50/50 p-2 rounded-xl"
              >
                <div class="flex sm:flex-col gap-1.5">
                  <button
                    @click=${() => this._moveCategory(index, -1, acciones)}
                    class="p-1.5 text-stone-400 hover:text-primary disabled:opacity-0 transition-all hover:scale-125"
                    ?disabled=${index === 0}
                  >
                    <svg
                      class="h-5 w-5 rotate-[-90deg] sm:rotate-0"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      stroke-width="3"
                ...
                <div class="flex gap-1.5 sm:hidden">
                  <button
                    @click=${() => this._prepareCatEdit(cat)}
                    class="p-2 text-primary bg-primary/5 rounded-lg transition-all active:scale-90"
                  >
                    <svg
                      class="h-5 w-5"
                      fill="none"
                  ...
                  <button
                    @click=${() => this._handleDeleteCategory(cat.id, acciones.onDeleteCategory)}
                    class="p-2 text-stone-300 bg-stone-100 rounded-lg transition-all active:scale-90"
                  >
                    <svg
                      class="h-5 w-5"
                      fill="none"
              ...
              <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0 w-full">
                <div
                  class="relative h-12 w-12 sm:h-14 sm:w-14 rounded-full bg-white flex items-center justify-center shrink-0 border-2 border-stone-100 overflow-hidden shadow-inner group-hover:border-primary/20 transition-all"
                >
                  <img
                    src="${cat.imageUrl || PLACEHOLDER_ICON}"
                    class="h-[60%] w-[60%] object-contain"
                    @error=${(e) => (e.target.src = PLACEHOLDER_ICON)}
                  />
                </div>

                <div class="flex-1 min-w-0">
                  <p
                    class="text-sm sm:text-base font-sans font-bold text-stone-900 uppercase tracking-wider truncate mb-0.5"
                  >
                    ${cat.nombre}
                  </p>
                  <div class="flex items-center gap-2">
                    <span
                      class="h-1.5 w-1.5 rounded-full ${cat.activo !== false
                        ? "bg-primary animate-pulse"
                        : "bg-stone-300"}"
                    ></span>
                    <span
                      class="text-[9px] sm:text-[10px] uppercase tracking-[0.1em] sm:tracking-[0.2em] ${cat.activo !==
                      false
              ...
              <div class="hidden sm:flex gap-2">
                <button
                  @click=${() => this._prepareCatEdit(cat)}
                  class="p-3 text-stone-400 hover:text-primary hover:bg-primary/5 rounded-2xl transition-all active:scale-95"
                >
                  <svg
                    class="h-5 w-5"
                    fill="none"
                ...
                <button
                  @click=${() => this._handleDeleteCategory(cat.id, acciones.onDeleteCategory)}
                  class="p-3 text-stone-300 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all active:scale-95"
                >
                  <svg
                    class="h-5 w-5"
                    fill="none"
  ...
  _renderCategoryForm(acciones) {
    return html`
      <form
        ...
        @submit=${(e) => this._handleCatSubmit(e, acciones)}
        class="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-stone-100 shadow-xl space-y-6 lg:sticky lg:top-8"
      >
        <div class="space-y-1 border-b border-stone-50 pb-5">
          <h4 class="text-stone-900 font-display italic text-base sm:text-lg">
            Añadir o Editar Categoría
          </h4>
          <p class="text-[7px] sm:text-[8px] text-stone-400 uppercase tracking-widest mt-0.5">
            Gestión de grupos
          </p>
        </div>
        ...
        <div class="space-y-5">
          <div>
            <label class="text-[9px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-2 block"
              >Nombre</label
            >
            <input
              type="text"
              ...
              class="w-full bg-stone-50 border-2 border-stone-100 rounded-xl py-3.5 px-5 text-stone-900 text-sm focus:border-primary transition-all outline-none"
              required
            />
          </div>
          <div>
          ...
          <label
            class="flex items-center gap-4 cursor-pointer group p-4 bg-stone-50 rounded-xl border-2 border-stone-100 hover:border-primary/30 transition-all active:bg-stone-100"
          >
            <input
              type="checkbox"
              id="new-cat-activo"
              checked
              class="w-5 h-5 rounded border-stone-200 bg-white text-primary focus:ring-0 focus:ring-offset-0"
            />
            <span
              class="text-[10px] sm:text-xs uppercase tracking-widest text-stone-500 group-hover:text-stone-900 transition-colors font-bold"
              >Estado Activo</span
            >
          </label>
        </div>
        <div class="flex flex-col gap-3 pt-2">
          <button
            type="submit"
            id="submit-cat-btn"
            class="w-full bg-stone-950 text-white py-5 sm:py-4 rounded-2xl text-xs sm:text-sm uppercase tracking-[0.4em] font-black hover:bg-primary transition-all shadow-xl active:scale-[0.97] transform"
          >
            Guardar Cambios
          </button>
          <button
            type="button"
            id="cancel-cat-edit"
            @click=${() => this._cancelCatEdit()}
            class="hidden w-full text-stone-400 py-3 text-[10px] sm:text-xs uppercase tracking-widest font-bold hover:text-stone-900 transition-all italic"
          >
            Cancelar Edición
          </button>
        </div>
  ...
  _renderProductGrid(platos, acciones) {
    if (!platos || platos.length === 0) {
      return html`<div
        class="py-16 text-center border-2 border-dashed border-stone-100 rounded-[2rem] text-stone-300 uppercase tracking-[0.4em] text-[10px] font-bold italic bg-stone-50/30"
      >
  ...
  _renderProductCard(p, acciones) {
    ...
    return html`
      <div
        class="p-4 rounded-3xl bg-white border border-stone-100 shadow-sm hover:shadow-xl transition-all duration-700 group flex flex-col gap-4 relative overflow-hidden"
        data-product-card
      ...
      >
        ${isSpecial
          ? html`<div class="absolute top-3 right-3 z-20">
              <span
                class="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[6px] font-black uppercase tracking-widest border border-amber-200"
                >Menú</span
              >
            </div>`
          : ""}
        ...
        <div class="flex gap-3 sm:gap-4 items-center relative z-10">
          <div
            class="relative h-16 w-16 sm:h-18 sm:w-18 shrink-0 rounded-xl overflow-hidden bg-stone-50 border border-stone-100"
          >
            <img
              src="${p.imageUrl || PLACEHOLDER_ICON}"
              class="h-full w-full object-cover group-hover:scale-110 transition-transform duration-[2s]"
              @error=${(e) => (e.target.src = PLACEHOLDER_ICON)}
            />
          </div>
          <div class="flex-1 min-w-0">
            <h4
              class="text-xs sm:text-sm font-sans font-bold text-stone-900 group-hover:text-primary transition-colors leading-tight uppercase truncate mb-0.5"
            >
              ${p.name}
            </h4>
            <p class="text-sm sm:text-base font-display italic text-stone-950">
              ${Number(p.price) > 0
                ? `S/ ${Number(p.price).toFixed(2)}`
                : html`<span class="text-stone-300 font-sans text-[10px]">S/ 0.00</span>`}
            </p>
          </div>
        </div>
        ...
        ${p.description
          ? html`<p
              class="text-[9px] sm:text-[10px] text-stone-500 leading-relaxed italic border-l-2 border-stone-100 pl-2 line-clamp-2"
            >
              ${p.description}
            </p>`
          : ""}
        ...
        <div class="flex flex-wrap gap-1 min-h-[16px]">
          ${cats.map(
            (c) =>
              html`<span
                class="px-2 py-1 rounded-md bg-stone-50 text-[7px] sm:text-[8px] font-bold text-stone-400 uppercase tracking-tighter border border-stone-100"
                >${c}</span
              >`
          )}
        </div>
        ...
        <div class="flex gap-2 pt-4 border-t border-stone-50 mt-auto">
          <button
            @click=${() => acciones.onEdit(p.id)}
            class="flex-1 py-2.5 rounded-xl bg-stone-50 text-stone-600 text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-stone-950 hover:text-white transition-all active:scale-95"
          >
            Editar
          </button>
          <button
            @click=${() => this._handleDeleteProduct(p.id, acciones.onDelete)}
            class="px-4 py-2.5 rounded-xl text-stone-300 hover:text-red-500 hover:bg-red-50 transition-all active:scale-95"
          >
  ...
  _renderProductForm(categorias, acciones) {
    ...
    return html`
      <form
        id="add-product-form"
        @submit=${(e) => this._handleProductSubmit(e, acciones)}
      ...
      >
        ...
        <div class="space-y-4 sm:space-y-6">
          <div>
            ...
            ><input
              ...
              class="w-full border-b-2 border-stone-100 py-2 sm:py-3 text-lg sm:text-xl font-display italic text-stone-900 focus:border-primary outline-none transition-all"
              required
            />
          </div>
          <div class="relative group">
            ...
            ><input
              ...
              placeholder="0.00"
              step="0.10"
              class="w-full border-b-2 border-stone-100 py-2 sm:py-3 text-xl sm:text-2xl font-black text-primary outline-none focus:border-primary transition-all disabled:opacity-20"
            />
            <p
              id="price-warning"
              class="hidden text-[7px] uppercase tracking-widest text-amber-600 font-bold mt-1.5"
            >
              Bloqueado para Menú
            </p>
          </div>
          <div>
            ...
            ><textarea
              id="new-description"
              rows="2"
              placeholder="Ingredientes, porción..."
              class="w-full bg-stone-50 border-none rounded-2xl p-4 text-sm text-stone-600 focus:ring-2 focus:ring-primary/20 outline-none transition-all"
            ></textarea>
          </div>
          <div>
            <label class="text-[9px] uppercase tracking-[0.3em] text-stone-400 font-bold mb-2 block"
            ...
            ><input
              ...
              id="new-image-url"
              placeholder="https://..."
              class="w-full bg-stone-50 border-none rounded-xl py-3 px-5 text-xs text-stone-900 focus:ring-2 focus:ring-primary/20 outline-none"
            />
          </div>
        </div>
        <div class="space-y-6 sm:space-y-8">
          <div
            class="p-5 sm:p-6 bg-amber-50/40 rounded-[1.5rem] sm:rounded-[2rem] border-2 border-amber-100 space-y-4"
          >
            <span class="text-[10px] uppercase tracking-[0.4em] text-amber-700 font-black"
              >Categorías Especiales</span
            >
            <div class="grid grid-cols-1 gap-2">${this._renderCategoryCheckboxes(specialCats)}</div>
          </div>
          <div
            class="p-5 sm:p-6 bg-stone-50 rounded-[1.5rem] sm:rounded-[2rem] border-2 border-stone-100 space-y-4"
          >
            <span class="text-[10px] uppercase tracking-[0.4em] text-stone-400 font-black block"
              >Carta General</span
            >
            <div class="grid grid-cols-1 gap-2">${this._renderCategoryCheckboxes(regularCats)}</div>
          </div>
        </div>
        <div class="flex flex-col gap-4 border-t border-stone-100 pt-8 sm:pt-10">
          <button
            ...
            class="w-full bg-stone-950 text-white py-6 sm:py-5 rounded-2xl text-xs sm:text-sm uppercase tracking-[0.4em] font-black shadow-xl hover:bg-primary transition-all duration-500 active:scale-[0.97] transform"
          >
            Guardar Producto
          </button>
          <button
      ...
      (cat) => html`
        ...
        >
          ...
          <input
            ...
            value="${cat.nombre}"
            class="w-5 h-5 rounded border-stone-200 text-primary focus:ring-0 disabled:pointer-events-none"
            @change=${(e) => this._handleCategoryChange(e)}
          />
        </label>
      `
    );
  ...
  async _handleDeleteCategory(id, onDelete) {
    if (await dialog.confirm("Eliminar Categoría", "¿Deseas eliminar esta categoría?"))
      onDelete(id);
  }
  ...
  prepareEdit(plato) {
    ...
    checkboxes.forEach((cb) => {
      const isChecked = Array.isArray(plato.category)
