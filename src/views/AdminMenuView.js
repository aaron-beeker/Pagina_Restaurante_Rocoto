    ...
    const template = html`
      <div class="min-h-screen bg-[#fafafa] font-sans">
        <div class="max-w-6xl mx-auto px-4 py-8 sm:py-16">
          ...
          <header
            class="flex flex-col sm:flex-row sm:items-end justify-between gap-8 mb-16 pb-8 border-b border-stone-200"
          >
            <div class="space-y-6">
              <button
                @click=${onBack}
            ...
            <div class="flex items-center gap-4 w-full sm:w-auto">
              ...
              <button
                @click=${() => onToggleVisibility(!activo)}
                class="h-14 px-6 flex items-center gap-3 rounded-2xl transition-all shadow-xl group border-2 ${activo
                  ? "bg-emerald-50 border-emerald-100 text-emerald-700 hover:bg-emerald-100"
                  : "bg-stone-100 border-stone-200 text-stone-400 hover:bg-stone-200"}"
              >
                <div
                  class="relative w-10 h-6 bg-stone-200 rounded-full p-1 transition-colors ${activo
                    ? "bg-emerald-500"
                    : ""}"
                >
                  <div
                    class="w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform ${activo
                      ? "translate-x-4"
                      : ""}"
                  ></div>
                </div>
                <span class="text-[10px] font-black uppercase tracking-[0.2em]"
                  >${activo ? "Visible en Web" : "Oculto en Web"}</span
                >
              </button>
              ...
              <button
                id="download-pdf-carta"
                class="h-14 px-6 flex items-center gap-3 bg-stone-950 text-white rounded-2xl hover:bg-primary transition-all shadow-xl group"
              >
                <svg
          ...
          <form
            id="admin-menu-form"
            @submit=${(e) => this._handleSubmit(e, onSave)}
            class="space-y-20 pb-32"
          >
            <div id="steps-container" class="space-y-24">${this._renderAllSteps("")}</div>

            <!-- Acción Principal Flotante -->
            <div class="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-50">
              <button
                type="submit"
                class="w-full bg-stone-950 text-white py-6 rounded-full text-[10px] uppercase tracking-[0.5em] font-black shadow-2xl hover:bg-primary transition-all duration-500 active:scale-95 group"
              >
                Actualizar Menú Público
                <span class="inline-block ml-4 group-hover:translate-x-1 transition-transform"
                  >&rarr;</span
                >
              </button>
            </div>
  ...
  _handleSubmit(e, onSave) {
    ...
    if (entradas.length === 0 || segundos.length === 0 || refrescos.length === 0) {
      import("../utils/notifications.js").then(({ toast }) => {
        toast.info(
          "Para actualizar, debes seleccionar al menos una Entrada, un Plato de Fondo y un Refresco."
        );
      });
      return;
    }
    ...
    return html`
      <section class="space-y-8">
        <div class="flex items-center gap-6">
          <h3
            class="text-[11px] font-black uppercase tracking-[0.4em] text-stone-300 whitespace-nowrap"
          >
            ${title}
          </h3>
          <div class="h-px w-full bg-stone-100"></div>
          <span
            class="text-[10px] font-black text-primary bg-primary/5 px-3 py-1 rounded-full border border-primary/10"
            >${list.length}</span
          >
        </div>
  ...
  renderCheckbox(plato, name) {
    ...
    return html`
      <label
        ...
        class="group relative flex cursor-pointer items-center justify-between p-6 rounded-2xl rounded-l-none bg-white border-2 border-stone-50 ${colorClass} border-l-4 hover:border-primary/20 hover:shadow-xl transition-all duration-500 active:scale-[0.98] has-[:checked]:border-primary"
      >
        <input
          type="checkbox"
          name="${name}"
          value="${plato.name}"
          class="peer sr-only"
          .checked=${plato.selected}
        />

        <div class="flex-1 min-w-0 pr-4">
          <p
            class="text-xs sm:text-sm font-sans font-bold text-stone-800 group-hover:text-primary transition-colors leading-tight uppercase"
          >
            ${plato.name}
          </p>
        </div>
        ...
        <div
          class="flex h-8 w-8 items-center justify-center rounded-2xl border border-stone-100 bg-stone-50 text-white peer-checked:bg-primary peer-checked:border-primary transition-all duration-500 shadow-inner group-hover:scale-110"
        >
          <svg
            class="h-4 w-4 opacity-0 peer-checked:opacity-100 transition-opacity"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            stroke-width="3"
