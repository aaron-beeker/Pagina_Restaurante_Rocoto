          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div id="banners-list" class="grid grid-cols-1 gap-4">
                    ${this.banners.length === 0 ? `
                        <div class="py-20 text-center border-2 border-dashed border-surface-variant rounded-3xl text-on-surface-variant opacity-40">
                            No hay banners configurados.
                        </div>
                    ` : this.banners.map((b, index) => `
                        <div class="group relative overflow-hidden rounded-2xl border border-surface-variant bg-surface shadow-sm hover:shadow-md transition-all">
                            <div class="flex flex-col sm:flex-row gap-4 p-4">
                                <div class="flex gap-2 shrink-0">
                                  <div class="w-32 aspect-video rounded-xl overflow-hidden bg-background border border-surface-variant">
                                      <img src="${escapeHtml(b.imageUrl)}" class="h-full w-full object-cover" />
                                  </div>
                                  <div class="w-16 aspect-[9/16] rounded-xl overflow-hidden bg-background border border-surface-variant">
                                      <img src="${escapeHtml(b.mobileImageUrl || b.imageUrl)}" class="h-full w-full object-cover" />
                                  </div>
                                </div>
                                <div class="flex-1 min-w-0 py-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="px-2 py-0.5 rounded-full bg-primary/5 text-[10px] font-bold text-primary uppercase tracking-tighter">Posición ${index + 1}</span>
                                        ${b.activo ? '<span class="px-2 py-0.5 rounded-full bg-green-50 text-[10px] font-bold text-green-600 uppercase tracking-tighter">Activo</span>' : ''}
                                    </div>
                                    <h4 class="font-bold text-on-background truncate">${escapeHtml(b.titulo || 'Banner')}</h4>
                                    <p class="text-[10px] text-on-surface-variant opacity-60 truncate mt-1">D: ${escapeHtml(b.imageUrl)}</p>
                                    <p class="text-[10px] text-on-surface-variant opacity-60 truncate">M: ${escapeHtml(b.mobileImageUrl || 'Misma que desktop')}</p>
                                </div>
            ...
            <div class="space-y-6">
                <div class="${adminShell.mutedBox}">
                    ...
                    <form id="hero-promo-form" class="space-y-4">
                        <label class="flex cursor-pointer items-center gap-3 mb-4">
                            ...
                            <span class="text-sm font-bold text-on-background">Activo</span>
                        </label>
                        <input type="text" id="hero-promo-titulo" class="${form.input}" placeholder="Título (Referencia interna)" />
                        <div>
                ...
                <div class="rounded-2xl border border-amber-100 bg-amber-50/50 p-6 text-amber-900">
                    <h4 class="text-[10px] font-bold uppercase tracking-widest mb-3 opacity-60">Guía de Tamaños</h4>
